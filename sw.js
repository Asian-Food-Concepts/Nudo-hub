const CACHE = 'nudo-hub-v112'; // v0.209.3: logo oficial Nudo en el header // v0.209.2: Seguimiento split into Compras & Mantenimiento tabs with tailored shapes, stepper pipeline & follow-up wiring // v0.209.1: proximo seguimiento wired; duration null fix // v0.209: Mantenimiento storyline: seguimiento por equipo, vista de abiertos, registro de costo y reparador // v0.208: Sobres category is optional // v0.207: Bitacora - new Sobres category // v0.206: Planear horario: slot rows derived from PLAN_MIN, Por puesto / Por persona tabs, inline coverage badges, collapsible warnings, separate error container // v0.205: AGF removed — management gate is dueño OR gerente (role only) // v0.204: Nuevos ingresos edit modes, document ticks, send-back & hide; Compras PDF removed; improved date pickers // v0.203: Solicitud de compras unidades + monto por unidad & Visto como deshabilitado para todos // v0.202: Nuevos ingresos contract PDFs (revisión y firma), quick-edit modal & send-back mode B // v0.201: CUTOVER — the repo now carries a CNAME file, so GitHub Pages
...
self.addEventListener('fetch', e => {
  ...
  if (req.mode === 'navigate') {
    ...
    e.respondWith(
      fetch(req, { cache: 'reload' }).catch(() => caches.match('./app.html'))
    );
    return;
  }
